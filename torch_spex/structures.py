...
def ase_atoms_to_tensordict(atoms_list : List[ase.Atoms], device : torch.device = "cpu") -> Dict[str, torch.Tensor]:
    ...
    atomic_structures["cells"] = torch.empty((n_structures, 3, 3), dtype=torch.get_default_dtype(), device=device)
    ...
    for structure_index, atoms in enumerate(atoms_list):
        ...
        atomic_structures["cells"][structure_index] = torch.tensor(atoms.cell.array, dtype=torch.get_default_dtype(), device=device)
